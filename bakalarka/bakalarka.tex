...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=0.7\columnwidth]{imgs/point_cloud.jpg}
	\end{center}
	\caption{Ukázka mapy získané z mračna bodů.}
...
\subsection{Uložení trajektorie}
...
\newpage
\subsection{Porovnání dat nahraných robotem}
...
\indent Pro zkoumání vlivu nastavení na systém, byly postupně měněny různé parametry. Jako testovací data byl zvolen zkrácený dataset z MIT Stata Center.\\
\indent Kritériem pro testování vlivu nastavení na algoritmus byla pro tento případ zvolena odlišná rovnice, která porovnává přímo polohu jednotlivých bodů trajektorie s $ground$ $truth$. Důvodem je absence extrahovatelné trajektorie přímo ze systému gMapping. Při vývoji tak nebyl kladen na typ srovnání dle (Kummerle), ale hlavně na to, aby vybrané částice tvořící mapu měli správné umístění. Rovnice použitá pro srovnání je ve tvaru
\begin{equation}
...
\indent Z těchto výsledků lze dojít k závěru, že systém je defaultně dobře nastavený na výkonější zařízení. Jednotlivá nastavení lze upravit tak, aby algoritmus poskytnul lepší výsledek, při kombinaci více parametrů dohromady však již vzniká riziko zvýšení výkonnových nároků natolik, že má změna nastavení na program opačný vliv.

\newpage
\section{Návrh změn v systému}
