...
#include "ros/ros.h" ///opt/ros/kinetic/include/ros/ros.h ... import ros
...
//tf::Transform() ... a representation of a translation and rotation
//tf::createQuaternionFromRPY(double roll, double pitch, double yaw) ... construct a Quaternion from Fixed angles
//tf::Point() ... the transform library representation of a point(Position)
SlamGMapping::SlamGMapping(): //from slam_gmapping.h from class SlamGmapping
...
void SlamGMapping::init()
{
  ...
  ROS_ASSERT(gsp_); // ROS_ASSERT(cond) ... Asserts that the provided condition evaluates to true
  ...
  // Parameters used by our GMapping wrapper
  if(!private_nh_.getParam("throttle_scans", throttle_scans_)) //process 1 out of every this many scans (set it to a higher number to skip more scans)
    throttle_scans_ = 1;
  if(!private_nh_.getParam("base_frame", base_frame_)) //the frame attached to the mobile base
    base_frame_ = "base_link";
  if(!private_nh_.getParam("map_frame", map_frame_)) //the frame attached to the map
    map_frame_ = "map";
  if(!private_nh_.getParam("odom_frame", odom_frame_)) //the frame attached to the odometry system
    odom_frame_ = "odom";

  private_nh_.param("transform_publish_period", transform_publish_period_, 0.05); //time between tranform publications (seconds)

  double tmp;
  if(!private_nh_.getParam("map_update_interval", tmp)) //time between updates to the map (seconds)
    tmp = 5.0;
  ...
  if(!private_nh_.getParam("minimumScore", minimum_score_)) //minimum score for considering the outcome of the scan matching good. Can avoid jumping pose estimates in large open spaces when using laser scanners with limited range (e.g. 5m). Scores go up to 600+, try 50 for example when experiencing jumping estimate issues
    minimum_score_ = 0;
  if(!private_nh_.getParam("sigma", sigma_)) //used by the greedy endpoint matching. Standard deviation for the scan matching process (cell)
    sigma_ = 0.05;
  if(!private_nh_.getParam("kernelSize", kernelSize_)) //the kernel in which to look for a correspondence
    kernelSize_ = 1;
  if(!private_nh_.getParam("lstep", lstep_)) //the optimization step in translation (linear)
    lstep_ = 0.05;
  if(!private_nh_.getParam("astep", astep_)) //the optimization step in rotation (angular)
    astep_ = 0.05;
  if(!private_nh_.getParam("iterations", iterations_)) //the number of iterations of the scanmatcher
    iterations_ = 5;
  if(!private_nh_.getParam("lsigma", lsigma_)) //the sigma of a beam used for likelihood computation
    lsigma_ = 0.075;
  if(!private_nh_.getParam("ogain", ogain_)) //gain to be used while evaluating the likelihood, for smoothing the resampling effects
    ogain_ = 3.0;
  if(!private_nh_.getParam("lskip", lskip_)) //number of beams to skip in each scan. Take only every (n+1)th laser ray for computing a match (0 = take all rays)
    lskip_ = 0;
  if(!private_nh_.getParam("srr", srr_)) //linear noise component (x and y). Odometry error in translation as a function of translation (rho/rho)
    srr_ = 0.1;
  if(!private_nh_.getParam("srt", srt_)) //linear -> angular noise component. Odometry error in translation as a function of rotation (rho/theta)
    srt_ = 0.2;
  if(!private_nh_.getParam("str", str_)) //angular -> linear noise component. Odometry error in rotation as a function of translation (theta/rho)
    str_ = 0.1;
  if(!private_nh_.getParam("stt", stt_)) //angular noise component (theta). Odometry error in rotation as a function of rotation (theta/theta)
    stt_ = 0.2;
  if(!private_nh_.getParam("linearUpdate", linearUpdate_)) //process a scan each time the robot translates this far
    linearUpdate_ = 1.0;
  if(!private_nh_.getParam("angularUpdate", angularUpdate_)) //process a scan each time the robot rotates this far
    angularUpdate_ = 0.5;
  if(!private_nh_.getParam("temporalUpdate", temporalUpdate_)) //process a scan if the last scan processed is older than the update time in seconds. A value less than zero will turn time based updates off
    temporalUpdate_ = -1.0;
  if(!private_nh_.getParam("resampleThreshold", resampleThreshold_)) //threshold at which the particles get resampled. Higher means more frequent resampling
    resampleThreshold_ = 0.5;
  if(!private_nh_.getParam("particles", particles_)) //number of particles. Each particle represents a possible trajectory that the robot has traveled
    particles_ = 30;
  if(!private_nh_.getParam("xmin", xmin_)) //initial map size (metres)
    xmin_ = -100.0;
  if(!private_nh_.getParam("ymin", ymin_)) //initial map size (metres)
    ymin_ = -100.0;
  if(!private_nh_.getParam("xmax", xmax_)) //initial map size (metres)
    xmax_ = 100.0;
  if(!private_nh_.getParam("ymax", ymax_)) //initial map size (metres)
    ymax_ = 100.0;
  if(!private_nh_.getParam("delta", delta_)) //resolution of the map (in metres per occupancy grid block). Size of one pixel
    delta_ = 0.05;
  if(!private_nh_.getParam("occ_thresh", occ_thresh_)) //threshold on gmapping's occupancy values. Cells with greater occupancy are considered occupied (i.e., set to 100 in the resulting sensor_msgs/LaserScan)
    occ_thresh_ = 0.25;
  if(!private_nh_.getParam("llsamplerange", llsamplerange_)) //translational sampling range for the likelihood
    llsamplerange_ = 0.01;
  if(!private_nh_.getParam("llsamplestep", llsamplestep_)) //translational sampling step for the likelihood
    llsamplestep_ = 0.01;
  if(!private_nh_.getParam("lasamplerange", lasamplerange_)) //angular sampling range for the likelihood
    lasamplerange_ = 0.005;
  if(!private_nh_.getParam("lasamplestep", lasamplestep_)) //angular sampling step for the likelihood
    lasamplestep_ = 0.005;
...
{
  //NodeHandle::advertise() methods are used to create a ros::Publisher which is used to publish on a topic
  //advertiseService() works very similar to how the subscribe() method works, in that you provide a service name and a callback to be invoked when the service is called
  //registerCallback() returns a message_filters::Connection object that allows you to disconnect the callback by calling its disconnect() method. You do not need to store this connection object if you do not need to manually disconnect the callback
  //boost::thread class is responsible for launching and managing threads
...
SlamGMapping::~SlamGMapping() // "~" destructor
{
...
SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
  ...
  try
  {
    //transform a stamped pose message into the target frame
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if(addScan(*scan, odom_pose))
  {
    ...
    // mutex ... sync means - It prevents two (or more) critical codes from being 
    // applied simultaneously over the same shared resource, such as global variable
...
// compute all possibilities of robot position
double
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  // RAII-style mechanism for owning one or more mutexes for the duration of a scoped block.
  // RAII - a programming idiom used to describe a particular language behavior
